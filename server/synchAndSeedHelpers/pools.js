...
const { models: { Pool, G1 } } = require('../db');
const fs = require("fs");
...
// going to copy this but can probably just make it a single function at some point 
...
const g1Streamer = async(csv, date) => {
  let streamPools = fs.createReadStream(csv)
  let csvPools = [];
  let csvStreamPools = fastcsv
  .parse()
  ...
  .on("end", async function() {
    for (let i = 1; i < csvPools.length; i++ ){
      ...
      try {
       // cusip,name,issuedate,currface,cfincmo,cfinfed,cfinplat,coupon,gwac,wala,wam,va,cprprediction,cprpredictionnext,date
      await G1.create({ cusip, name, indicator, type, issuedate, currentface, cfincmo, cfinfed, cfinplat, coupon, gwac, wala, wam, va, pastactcpr, 
                          curractualcpr, cprprediction, cprpredictionnext, pastactcdr, curractualcdr, cdrprediction, cdrpredictionnext, date })
      }
      catch(ex){
        console.log(ex)
